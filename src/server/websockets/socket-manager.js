        ...
        socket.on('camera-control', (data) => {
            ...
            if (!camera) {
                ...
                socket.emit('camera-control-error', {
                    cameraId,
                    error: 'Camera not found. Please refresh the page and try again.'
                });
                return;
            }
            ...
            if (String(camera.userId) !== String(userId)) {
                ...
                socket.emit('camera-control-error', {
                    cameraId,
                    error: 'Unauthorized access to camera'
                });
                return;
            }
            
            // Check if camera WebSocket is available
            if (!camera.ws || camera.ws.readyState !== 1) { // 1 = OPEN
                console.log(`❌ Camera ${cameraId} WebSocket not available for control (readyState: ${camera.ws?.readyState})`);
                socket.emit('camera-control-error', {
                    cameraId,
                    error: 'Camera not connected. Please wait for camera to reconnect.'
                });
                return;
            }
            ...
            try {
                ...
                socket.emit('camera-control-sent', {
                    cameraId,
                    command,
                    settings,
                    timestamp: Date.now()
